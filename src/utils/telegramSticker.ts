...
function getToken(): string {
  ...
  if (!token) {
    throw new Error(
      'Token Telegram bot tidak ditemukan. Set TSTICKERS_TOKEN atau TELEGRAM_BOT_TOKEN di .env (buat via @BotFather).',
    );
  }
  return token;
}
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    child.on('error', (error: NodeJS.ErrnoException) => {
      ...
      if (error.code === 'ENOENT') {
        reject(new Error(options.notFoundMessage || `Perintah "${command}" tidak ditemukan.`));
        return;
      }
      reject(error);
    });
    ...
    child.on('close', (code) => {
      ...
      clearTimeout(timer);
      if (timedOut) {
        reject(new Error(`Perintah "${command}" timeout setelah ${options.timeoutMs}ms.`));
        return;
      }
      if (code === 0) {
        resolve();
...
async function runTStickers(packInput: string, token: string, workDir: string): Promise<void> {
  ...
  await runProcess(getBinary(), ['-p', packInput], {
    ...
    notFoundMessage:
      'tstickers tidak ditemukan. Install dulu dengan "python -m pip install tstickers" atau set TSTICKERS_BIN.',
  });
}
...
async function convertWebmToAnimatedWebp(inputPath: string, outputPath: string): Promise<void> {
  ...
  await runProcess(getFfmpegBin(), args, {
    ...
    notFoundMessage:
      'ffmpeg tidak ditemukan. Install ffmpeg dan pastikan ada di PATH, atau set FFMPEG_BIN untuk konversi webm → webp animasi.',
  });
}
...
export async function downloadTelegramStickerPack(
  ...
  if (!packUrl) {
    throw new Error('URL pack Telegram tidak diberikan.');
  }
  ...
  try {
    ...
    log.debug(`[TelegramSticker] "${sourceLabel}": ${allFiles.length} file dihasilkan oleh tstickers.`);
    ...
    if (stickerFiles.length === 0) {
      throw new Error(
        `tstickers berhasil jalan untuk "${sourceLabel}", tapi tidak ada file sticker (.webp/.webm) yang ditemukan di hasil output.`,
      );
    }
    ...
    if (stickerBuffers.length === 0) {
      throw new Error(
        'Semua file sticker dari pack Telegram gagal dibaca atau melebihi batas ukuran 1MB.',
      );
    }
